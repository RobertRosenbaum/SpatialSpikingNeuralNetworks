...
sig1=dt*conv(randn(Nt,1)./sqrt(dt),Kx,'same'); % white noise convolved with Kx
Ix1e=mxe+sig1;
Ix1i=mxi+sig1;

% New, independent realization for pop 2
sig2=dt*conv(randn(Nt,1)./sqrt(dt),Kx,'same'); % white noise convolved with Kx
Ix2e=mxe+sig2;
Ix2i=mxi+sig2;
...
t0=toc;
disp(sprintf('Time for 1fgh simulation=%d sec',round(t0)))

save NetworkSimForFigure1fgh.mat
